import { visit, click, fillIn, triggerKeyEvent, focus, blur, find } from 'react-qunit-test-helpers';
...
module('Acceptance | Field methods', function (hooks) {
  ...
  test('pushErrors method - field unvalidated when called', async function (assert) {
    await visit('docs/field-methods');
    assert.notOk(await wasValidated('[data-test-id="field-methods7-form-name-field"]', validationTestHelpersDefaults), 'Field is not validated on insert.');
...
async function checkPushErrors(assert, opts = {}) {
  await fillIn('[data-test-id="example-7"] [data-test-id="error-message-input"]', 'One pushed error message');
  await blur('[data-test-id="example-7"] [data-test-id="error-message-input"]');
  await failedValidation(`[data-test-id="field-methods7-form-name-field"]`, validationTestHelpersDefaults, assert, 'The pushErrors field method works when field has not yet been validated when it is called.');
  await fillIn('[data-test-id="example-7"] [data-test-id="error-message-input"]', 'Another pushed error message');
  await blur('[data-test-id="example-7"] [data-test-id="error-message-input"]');
  assert.dom('[data-test-id="field-methods7-form-name-field"] [data-test-class="cwf-field-errors"]').hasText(`${opts.existingErrors ? opts.existingErrors.join(' ') + ' ' : ''}One pushed error message Another pushed error message`, 'Calling pushErrors again adds to the existing errors.');
}

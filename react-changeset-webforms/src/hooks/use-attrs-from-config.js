...
import { classNamesFromConfig, mergedAttrFunctions, setCustomValidity } from 'validated-changeset-webforms';
...
export default function useAttrsFromConfig(ref, names, changesetWebform, formField) {
  ...
  useEffect(() => {
    ...
    const classNames = classNamesFromConfig(names, changesetWebform, formField, element);
    element.classList.remove(...classNames.filter((c) => c.startsWith('!')).map((c) => c.slice(1)));
